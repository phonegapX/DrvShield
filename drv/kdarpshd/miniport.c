...
BOOLEAN CheckPacket(PCHAR lpBuffer, PCHAR lpMac)
{
	BOOLEAN bResult = TRUE;
	switch(NTOHS(((PETH_HDR)(lpBuffer))->eth_type))
	{
	case ETH_TYPE_ARP:
		{
			PARP_PACKET arpPacket = (PARP_PACKET)lpBuffer;
			switch(NTOHS(arpPacket->ar_hdr.ar_op))
			{
			case ARP_OP_REQUEST:
				{
					if ( 
						memcmp(arpPacket->eh_hdr.eth_src, lpMac, ETH_ADDR_LEN) != 0 ||
						memcmp(arpPacket->ar_hdr.ar_sha , lpMac, ETH_ADDR_LEN) != 0
						) 
					{
						bResult = FALSE;	
					}
			...
			case ARP_OP_REPLY:
				{
					if ( 
						memcmp(arpPacket->eh_hdr.eth_src, lpMac, 6) != 0 ||
						memcmp(arpPacket->ar_hdr.ar_sha , lpMac, 6) != 0
						) 
					{
						bResult = FALSE;	
					}
...
--*/
{
    ...
    for (i = 0; i < NumberOfPackets; i++)
    {
		...
		// ARP过滤
		{
			PCHAR PacketVirtualAddress;		//由于ARP包小,不用获取下一个MDL
			NdisQueryBufferSafe(Packet->Private.Head, &PacketVirtualAddress, &pAdapt->uSizeOfBuffer, HighPagePriority);
			NdisMoveMemory(pAdapt->Buffer, PacketVirtualAddress, pAdapt->uSizeOfBuffer);
		}
		if (!CheckPacket(pAdapt->Buffer, pAdapt->MacAddr))
		{
